...
describe("Paddle", () => {
	...
	it("moves left when left is pressed", async () => {
		const result = await runScene(Level1, InputScript.create().press("left", 30), 0.5);
		const paddle = result.game.currentScene!.findByType(Paddle);
		expect(paddle!.position.x).toBeLessThan(GAME_WIDTH / 2);
	});

	it("stops at left wall", async () => {
		const result = await runScene(Level1, InputScript.create().press("left", 180), 3);
		const paddle = result.game.currentScene!.findByType(Paddle);
	...
	it("stops at right wall", async () => {
		const result = await runScene(Level1, InputScript.create().press("right", 180), 3);
		const paddle = result.game.currentScene!.findByType(Paddle);
